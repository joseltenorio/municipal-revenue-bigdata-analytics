...
CREATE EXTERNAL TABLE IF NOT EXISTS `bronze`.`municipal_classification` (
  `anio` BIGINT,
  ...
  `bronze_source_url` STRING,
  `bronze_source_page_url` STRING,
  `bronze_processed_at_utc` STRING
)
STORED AS PARQUET
LOCATION '/app/data/bronze/municipal_classification';

CREATE EXTERNAL TABLE IF NOT EXISTS `bronze`.`renamu` (
  `ano` STRING,
  `idmunici` STRING,
  `ccdd` STRING,
  `ccpp` STRING,
...
STORED AS PARQUET
LOCATION '/app/data/bronze/renamu';

CREATE EXTERNAL TABLE IF NOT EXISTS `bronze`.`siaf_income` (
  `ano_doc` STRING,
  `mes_doc` STRING,
...
STORED AS PARQUET
LOCATION '/app/data/bronze/siaf_income';

CREATE EXTERNAL TABLE IF NOT EXISTS `bronze`.`sismepre` (
...
STORED AS PARQUET
LOCATION '/app/data/bronze/sismepre';
