"""Descarga controlada de la fuente MEF/SISMERE de SISMEPRE.
...
def get_candidate_resources(source_config: dict[str, Any]) -> dict[str, Any]:
    """Obtiene recursos candidatos definidos para sismepre."""

    resources = source_config.get("candidate_resources", {})

    if not resources:
        raise IngestionError(
            "No existen candidate_resources para sismepre en config/sources.yaml."
        )
...
def parse_args() -> argparse.Namespace:
    ...
    parser = argparse.ArgumentParser(
        description="Descarga recursos de SISMEPRE hacia Landing."
    )
    parser.add_argument(
        "--resource",
